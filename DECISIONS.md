...
## 2026-06-17 — Remove the multi-theme system; one fixed palette
**Choice:** Delete the selectable themes + terminal-background detection (6 palettes, `VANTA_THEME`, COLORFGBG/OSC-11 auto-detect, `resolveThemeByName`/`THEME_NAMES`/`currentThemeName`, the `/theme` command + overlay, the setup section, `term/osc-detect.ts`). Collapse to ONE fixed palette (`THEME` in `term/theme.ts`, terminal-native white/gray) still served via `ThemeProvider`/`useTheme` so the UI keeps its token structure (colours in one place, not hardcoded across ~15 components).
**Why:** Jason: "remove the themes, they're useless." The detection was flaky (the only red tests in the suite were theme-detection assertions), multiple palettes went unused, and keeping the token abstraction (one palette behind a context) is the modular choice — removing it would mean hardcoding colours everywhere. Net: suite went fully green (3524/3524), commands 98→97.
**Reversible?** Yes — recoverable from git history; but detection was the flaky part, so don't restore it.
