...
**Scope (cards):** 28 `PCLIP-*` cards were extracted from Paperclip (github.com/paperclipai/paperclip, MIT) into roadmap.json. The in-bounds reliability and single-owner fan-out work shipped. `PCLIP-MULTI-COMPANY` and `PCLIP-MULTI-USER` were retired from the active roadmap on 2026-07-11 because they require reversing this decision; `PCLIP-SANDBOX-AGENTS` shipped as a single-owner remote-backend adapter. The retirement rationale lives in `docs/roadmap-retired-cards.md`.
...
**Reversible?** Yes — framing lock; a deliberate strategy reversal can create new tenancy cards with current proof criteria. The rejected cards are not active backlog.
...
## 2026-08-18 — Zero-new-spend user testing and roadmap work
**Choice:** Vanta roadmap execution and user testing use voluntary participation plus existing owner-controlled local or open-source tooling. No participant payment, paid recruiting, paid research platform, paid GitHub Actions, paid hosting, paid outreach automation, or other new service spend is authorized. `DESKTOP-COLD-OPERATOR-RELEASE-PROOF` becomes the sole Next card; `GROW-01` moves to Horizon behind it and no longer requires paid co-design.
**Why:** Jason explicitly said, “we arent paying anyone for any of this,” while prioritizing user testing over additional Trust implementation. The plan must represent that constraint instead of leaving a paid evidence contract as the active queue.
**Evidence boundary:** Voluntary, uncompensated participation can reveal usability failures but may be selection-biased and is not representative demand or pricing proof. Record that limitation, informed consent, assistance, drop-off, and observed outcomes. Do not convert one friendly run into a market claim.
**Supersedes:** Only the paid-participation requirement in the 2026-07-30 realignment. Disability-led and neurodivergent requirements remain non-negotiable source constraints; diagnosis inference and diagnosis gating remain prohibited.
**Reversible?** A later explicit owner decision may authorize a bounded expense. No system or roadmap item may infer that authority.
