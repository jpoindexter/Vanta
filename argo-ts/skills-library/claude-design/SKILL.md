...
Use this skill when the user asks for design work that would normally fit a hosted design tool, but the agent is running in a CLI/API environment instead of a hosted design web UI.

The goal is to preserve good design behavior and taste while removing hosted-tool plumbing that does not exist in normal agent environments.

If the user wants a known brand's look, supply the visual vocabulary from a real product reference. If the deliverable is a design-token spec file rather than a rendered artifact, author the token spec instead of an HTML artifact.

## Runtime Mode

You are running in **CLI/API mode**, not a hosted design web UI.

Ignore references from hosted design prompts to hosted-only tools, project panes, preview panes, special toolbar protocols, or platform callbacks that are not available in the current environment.
...
- built-in hosted artifact-completion helpers
...
Do not use this skill for pure design-token spec authoring unless the user specifically asks for a token spec file.
...
A hosted design edit-mode toolbar does not exist here.
...
When adapting a hosted design style request into CLI/API mode, use this mental translation:

```text
You are running in CLI/API mode, not a hosted design tool. Ignore references to hosted-only tools or preview panes. Produce complete local design artifacts, usually self-contained HTML with embedded CSS/JS, and verify with available local tools before returning. Preserve the design process: gather context, define the system, produce options, avoid filler, and meet a high visual bar.
```
