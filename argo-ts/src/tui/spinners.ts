...
// bar. Pick the active one with ARGO_SPINNER (defaults to "orbit").
...
/** Frames for the configured spinner, honouring ARGO_SPINNER, with a fallback. */
export function spinnerFrames(env: NodeJS.ProcessEnv = process.env): readonly string[] {
  const name = env.ARGO_SPINNER as SpinnerName | undefined;
