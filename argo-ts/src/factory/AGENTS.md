...
`argo-ts/src/factory/*.ts` files are kernel-protected. Any write attempt returns `Risk::Block`.

## Tests

All pure logic has co-located unit tests (`*.test.ts`). Run: `cd argo-ts && npx vitest run src/factory/`
