...
describe("upsertEnv", () => {
  it("replaces an existing uncommented key in place", () => {
    const out = upsertEnv("ARGO_PROVIDER=ollama\nARGO_MODEL=qwen2.5:14b\n", {
      ARGO_PROVIDER: "gemini",
    });
    expect(out).toContain("ARGO_PROVIDER=gemini");
    expect(out).not.toContain("ARGO_PROVIDER=ollama");
    expect(out).toContain("ARGO_MODEL=qwen2.5:14b"); // untouched
  });

  it("preserves unrelated secrets — never regenerates", () => {
    const existing =
      "# comment\nARGO_PROVIDER=ollama\nARGO_GOOGLE_CLIENT_ID=abc\nSERPAPI_KEY=xyz\n";
    const out = upsertEnv(existing, { ARGO_PROVIDER: "gemini", GEMINI_API_KEY: "k" });
    expect(out).toContain("# comment");
    expect(out).toContain("ARGO_GOOGLE_CLIENT_ID=abc");
  ...
  it("appends new keys not already present", () => {
    const out = upsertEnv("ARGO_PROVIDER=openai\n", { OPENAI_API_KEY: "sk-1" });
    expect(out).toContain("ARGO_PROVIDER=openai");
    expect(out).toContain("OPENAI_API_KEY=sk-1");
  });

  it("does not treat a commented key as a real assignment", () => {
    const out = upsertEnv("# ARGO_PROVIDER=openai\n", { ARGO_PROVIDER: "gemini" });
    expect(out).toContain("# ARGO_PROVIDER=openai"); // comment kept
    expect(out).toContain("ARGO_PROVIDER=gemini"); // real key appended
  });

  it("handles an empty starting file and ends with one newline", () => {
    const out = upsertEnv("", { ARGO_PROVIDER: "gemini", ARGO_MODEL: "gemini-2.5-flash" });
    expect(out).toBe("ARGO_PROVIDER=gemini\nARGO_MODEL=gemini-2.5-flash\n");
  });
});
...
describe("buildEnvUpdates", () => {
  ...
  it("omits the key var for a keyless provider", () => {
    ...
    expect(u).toEqual({ ARGO_PROVIDER: "ollama", ARGO_MODEL: "qwen2.5:14b" });
...
describe("runSetup (integration)", () => {
  ...
  it("writes a merged .env from the wizard answers, preserving existing keys", async () => {
    const root = await tempRepo("ARGO_GOOGLE_CLIENT_ID=keepme\nARGO_PROVIDER=ollama\n");
    ...
    expect(env).toContain("ARGO_GOOGLE_CLIENT_ID=keepme"); // preserved
    expect(env).toContain("ARGO_PROVIDER=gemini"); // replaced
    expect(env).toContain("ARGO_MODEL=gemini-2.5-flash"); // default applied
    expect(env).toContain("GEMINI_API_KEY=secret-key");
    expect(env).not.toContain("ARGO_PROVIDER=ollama");
  });
