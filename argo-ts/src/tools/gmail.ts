...
/** Gmail header arrays look like [{ name, value }]; pick one by name (case-insensitive). */
 ...
 * `from` is left to "me" — Gmail fills the authenticated address.
 */
...
// Gmail message payloads nest recursively (multipart/alternative etc.); the
// text/plain body can be several levels deep, so we walk the whole tree.
type Payload = {
  mimeType?: string;
  body?: { data?: string };
  parts?: Payload[];
...
const PayloadSchema: z.ZodType<Payload> = z.lazy(() =>
  z.object({
...
/** Recursively find the first text/plain body; fall back to any top-level body. */
function extractPlainText(payload: Payload): string {
...
/** Turn a non-2xx Response into an errors-as-value result. */
async function httpError(res: Response): Promise<ToolResult> {
  const body = await res.text().catch(() => "");
  return { ok: false, output: `gmail HTTP ${res.status}: ${body.slice(0, 500)}` };
}

/** googleFetch throws the actionable "not authorized" message on auth/network failure. */
const authError = (err: unknown): ToolResult => ({
  ok: false,
  output: (err as Error).message,
});

// --- outbound shared args (draft + send differ only in body shape + endpoint) ---

const OutboundArgs = z.object({
  to: z.string().min(1),
...
const SearchListResponse = z
  .object({
    // `messages` is absent (not []) when nothing matches.
...
export const gmailSearchTool: Tool = {
  ...
  async execute(raw): Promise<ToolResult> {
    ...
    if (!parsed.success) {
      return { ok: false, output: 'gmail_search needs a "query" string (max optional, 1-25)' };
    }
...
/** Fetch one message's metadata and format "<id> | From | Subject | snippet". */
async function fetchMetadataLine(id: string): Promise<string> {
  // Repeated metadataHeaders params can't go through a k/v buildUrl, so build literally.
  ...
  return `${id} | ${from} | ${subject} | ${meta.snippet ?? ""}`;
}
...
export const gmailReadTool: Tool = {
  ...
  async execute(raw): Promise<ToolResult> {
    ...
    if (!parsed.success) {
      return { ok: false, output: 'gmail_read needs an "id" string' };
    }
    try {
...
export const gmailDraftTool: Tool = {
  ...
  async execute(raw, ctx: ToolContext): Promise<ToolResult> {
    ...
    if (!parsed.success) {
      return { ok: false, output: 'gmail_draft needs "to", "subject", and "body"' };
    }
    const approved = await ctx.requestApproval(
      "create a gmail draft",
      "creates a draft in your account",
    );
...
/** Send mail — approval-gated, irreversible, never auto-sends. */
export const gmailSendTool: Tool = {
  ...
  async execute(raw, ctx: ToolContext): Promise<ToolResult> {
    const parsed = OutboundArgs.safeParse(raw);
    if (!parsed.success) {
      return { ok: false, output: 'gmail_send needs "to", "subject", and "body"' };
    }
    const approved = await ctx.requestApproval(
      "send an email",
      "sends mail from your account — irreversible",
    );
    if (!approved) return { ok: false, output: "denied by user" };
