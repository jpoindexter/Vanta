...
| `mcp/mount.ts` | v1 E5 + MCP-1 — `readMcpConfig` (accepts `servers`+`mcpServers` keys; resolves `ARGO_MCP_SERVERS` inline, else merges `./.mcp.json` project-level over `~/.argo/mcp.json` user-level) + `mountMcpServers` (spawn each, register discovered tools as Argo tools via `mcpToolToArgoTool` — gated by kernel `assess()`). Called in `prepareRun`; no-op without config |
| `mcp/server.ts` | MCP-3 — Argo AS an MCP server (mirror of client.ts). `runMcpServer`/`stdioServerTransport` + pure `handleMessage` (initialize/tools/list/tools/call). Every call gated by `assess()`: `block`/`ask` → `isError` result (headless, no human), only `allow` executes. `resolveServeAllowlist` (`ARGO_MCP_SERVE_TOOLS`, default 9 read-only) bounds exposure. `argo mcp serve` |
