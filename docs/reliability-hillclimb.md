...
1. **Live completion needs external evidence.** At this historical checkpoint, Telegram + ntfy had accepted real round-trips. Do not infer that every other registered adapter needs only a token: each current adapter still needs its own provider, policy, account, and receipt review.
...
- **Caveat 2 (historical 20-adapter snapshot)** → the shared send/poll path was live-proven through Telegram bot-poll and ntfy pub/sub, and the then-current adapters had focused tests. This does not certify the current registry or reduce every remaining adapter to a credential-only gate. The current registry contains 22 adapters; only Telegram and ntfy have accepted live receipts in this repository.
