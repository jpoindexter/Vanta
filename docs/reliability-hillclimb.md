...
- **Caveat 2 (adapters offline-tested only)** → **path now live-proven on a real service; the rest is a token reality.** The `PlatformAdapter` contract is type-enforced; all 20 adapters have dedicated tests (**543 platform tests green**). Pushed it further: **live-verified the ntfy adapter against the real ntfy.sh** (no-token public topic — `scripts/adapter-live.sh`: real POST send + real GET poll, exact round-trip match). So **2/20 are now live-verified** (Telegram bot-poll + ntfy pub/sub — two *different* transport mechanisms), proving the send/poll FRAMEWORK works end-to-end on real external platforms. The other 18 are gated only by their platform token (operator-provided) — a credentials reality, not a code defect.
