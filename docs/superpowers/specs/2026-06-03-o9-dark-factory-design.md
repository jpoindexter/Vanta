...
> **Done = a scheduled (or `vanta improve`-triggered) cycle picks the highest-value work
...
- **On-demand** — `vanta improve` calls `factory/run.ts` inline and streams progress to the
  TUI. Same code path; `interactive: true`.

A lockfile (`~/.vanta/factory.lock`) prevents the gateway from double-spawning. `vanta improve`
...
the human's working branch. Fixes branch-collision with a live `vanta run` session and makes
...
`review` → `auto` graduation requires an explicit `vanta factory approve --promote` after a run
...
surfaced by `vanta approvals` / the TUI) for the human to clear at their next session — never
...
5. PLAN      build factory.plan.json (ONE slice).  review-mode → render + wait for `vanta factory approve`.  auto-mode → proceed
...
9. LOG       ~/.vanta/logs/factory-<ts>.log · token spend in the commit message · `vanta factory status` shows last result
```
...
- **`vanta factory logs --follow`** — nice-to-have; `status` + the log file cover v0.
