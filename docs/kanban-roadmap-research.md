...
- `argo roadmap` command builds + opens it. Vanta already **reads** the board natively.
...
1. **Vanta control — `roadmap_move` tool + `argo roadmap move <id> <status>`** (kernel-gated write to `roadmap.json` → regenerate html). Vanta moves cards as it works; the dark factory moves a card `next→building` on start and `building→shipped` on verified-commit. Closes the loop between the autonomy ladder and the board. *(size S)*
