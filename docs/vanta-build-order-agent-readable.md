...
001. [building] TRUST-02 — Urgent control-plane trust closure — hooks, environments, audit state, and local authentication
    track: Harness | tier: rock | size: L | effort: high | model: opus
    why: Close the smallest confirmed P0 slice before consequential autonomy: protect project control-plane state, prevent same-run hook activation, scrub shell/code/hook environments, deny ordinary credential and audit-key access, and authenticate every local Desktop/API instance.
    done: A model-controlled attempt to create or modify hook/control-plane state, inspect process/project credentials or audit signing state, or drive a local Desktop/API without exact authentication produces no unmediated host effect and no secret exposure; the complete proposed payload is denied or exact-approved; hook activation is restart-bounded; audit evidence remains outside agent scope; exact-origin validation and adversarial shell, file, hook, Desktop, and restart paths run end to end with retained receipts.

002. [building] UX-03 — Safe continuity slice — messy capture to Today, prepared action, waiting, and restart re-entry
    track: Operator | tier: rock | size: L | effort: high | model: opus
    why: Prove the wedge without consequential external authority: one bounded messy life-or-work item becomes a sourced outcome, one Today recommendation, a prepared action using the relevant generalist capability, durable waiting/resume state, and correct restart re-entry.
    done: On the supported launch path, a real local/read-only item is captured without forced taxonomy, yields one recommendation and at most three choices, prepares a useful action in under fifteen minutes, records owner/wait condition/resume context, survives a real process restart, and returns the last verified state plus one re-entry action without transcript reconstruction or repeated external effect. Capacity may be unknown across cognitive, attentional, sensory, social, emotional, physical, and time dimensions. Literal controls include do it, show me, snooze, skip, and off; refusal works for the session, repeated pattern, and global support; transient state expires or is reviewed; quiet hours, interruption budgets, stable layout, non-color meaning, reduced motion, optional streaming/auto-scroll, aphantasia-safe concrete previews, and shame-free recovery are proven.

003. [next] TRUST-04 — Truthful completion and one typed receipt across every host
    track: Harness | tier: rock | size: M | effort: high | model: opus | after: TRUST-02
    why: Make Desktop, CLI/TUI, messaging, jobs, memory, and goal progress consume one evidence-derived WorkItem/Run/Approval/Receipt contract rather than model prose or host-specific stop reasons.
    done: Every supported host persists exactly the WorkItem states draft, queued, running, waiting, needs human, stopped, failed, unverified, and verified. Captured, Now, Waiting, Needs You, and Done remain projections only. Action/receipt dispositions denied, expired, unknown, and compensated remain separate. Verification outages and missing readback produce unverified; unknown effects are never blindly retried; failed, stopped, denied, expired, unknown, and compensated attempts cannot create accomplishment memory; representative file, UI, message, calendar, job, and restart paths prove the contract end to end.

004. [next] TRUST-01 — Universal effect-path inventory and one trusted action gateway
...
005. [next] OP-01 — Minimum operator spine — WorkItem, Run, Approval, Receipt, follow-up, and resume
...
006. [next] GROW-01 — Phase-0 trusted-continuity evidence lane — interviews, pilots, burden, retention, and price
...
012. [horizon] LIFE-02 — Quarantined read-only morning orientation
