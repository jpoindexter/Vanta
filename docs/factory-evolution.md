...
Five Levels) against Vanta's own dark factory. **Adapt the ideas, never copy the code.** Roadmap
items derived below are tracked as `FAC-*`.

## The decisive distinction

Every external reference builds **new** software from specs (greenfield, spec→app). **Vanta improves
its OWN existing codebase (brownfield self-modification).** That filter decides what transfers — and
it's the reason Vanta is *ahead* on the one problem none of them face (self-modifying the factory/kernel).
...
- **Holdout author-separation** — the agent that writes a fix must not author its own acceptance check.
  Vanta's "new test must fail on old code" is a *partial* holdout; the hole is that the same cycle writes
...
- **Model escalation + per-cycle cost ledger** — answers Willison's ~$1k/day caution; serves frugality.
  Vanta has token budgets but no escalation or cost accounting. → `FAC-ESCALATE`.
...
- **Five-Levels** — positioning only (Vanta's autonomy ladder L1–L5 already mirrors L0→L5). Audit/manifesto.

### CAN'T USE / CUT (deliberately not building)
- **Digital Twin Universe** — irrelevant to self-improvement (Vanta isn't validating against Okta/Slack
...
- **Full Level-5 black-box autonomy** ("humans unnecessary") — *anti-thesis*: contradicts the ND lens
  (the operator/partner IS the point) and the kernel-safety thesis. Vanta is a **L4 collaborator + hard
...
### MISSING (Vanta gaps the sources don't cover — self-supply; some in flight)
- **Self-modification safety** — none asks "what if the factory breaks the factory/kernel?" Vanta's
  unique problem, addressed by O11 compartments + the `SR` propose→prove→swap design. Ahead; protect.
- **Two-layer trust** — they're one-layer (validation only). Vanta = kernel + validation. Holdout is a
...
Vanta = dark factory **+ enforced kernel boundary + ND collaborator stance + self-repair architecture**.
