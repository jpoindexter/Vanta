...
Purpose: map every Hermes slash command and what it DOES, so Argo can re-implement
...
Argo has **no gateway**. The port = collapse the fallthrough into in-process dispatch:
- Keep a single local command table (like the TS registry) — there is no step 3/4 round-trip.
- Every `ctx.gateway.rpc('session.*' | 'config.*' | …)` becomes a **direct call** into Argo's sessions store (`~/.argo/sessions`), provider abstraction, tool registry, or kernel.
- The `skill`/`send`/`prefill` distinction still matters: `send` = enqueue a real turn; `prefill` = populate the composer; `skill` = inject a skill-invocation prompt. Argo's composer + turn loop already model these.
- Commands marked **gateway_only** below (messaging-platform plumbing) are **Argo N/A** — skip them.
...
- **Argo port:** Provider list comes from Argo's **provider abstraction**; render the same 2-stage picker. Stage 1 lists configured providers (+auth state), stage 2 lists each provider's model IDs. Key entry writes to Argo's env/credential store. Switch = update active model on the current session in `~/.argo/sessions`. Drop the `--global` vs session flag down to "persist to config" vs "this session only".
...
- **Argo port:** Read `~/.argo/sessions` for the list; "activate/resume" loads that session's transcript into the TUI; "new" creates a session row; delete removes the dir/row. Argo likely runs one active session in-process — "live vs cold" can collapse to "currently loaded vs on disk" unless Argo supports concurrent sessions.
...
- **Argo port:** Maps to Argo's **kernel** (goals/subagents). If Argo doesn't do multi-level delegation yet, this is a future overlay — port the pause/kill/status semantics onto whatever Argo's task/goal runner exposes. `/replay` + `/replay-diff` (below) read archived spawn trees from disk via the same overlay.
...
- **Argo port:** Wire to Argo's **tool registry** / skill loader. Inline subcommands stay the same; the hub overlay is optional polish.
...
- **Argo port:** Argo can run its own setup as an in-process wizard screen OR suspend Ink and run a setup subcommand. Simpler in-process: a dedicated setup overlay that writes provider creds to `~/.argo` config, then `setup.status`-equivalent check.
...
- **Argo port:** New session row in `~/.argo/sessions`, clear in-memory transcript, keep the confirm overlay (it's a destructive op).

---

## 3. Non-interactive client-side commands (TS local registry)

These run in-process in the TUI today; they're the natural Argo in-process set. RPC calls listed = what becomes a direct Argo call.

| Command | Aliases | Cat | DOES | Backend RPC → Argo |
|---|---|---|---|---|
| `/redraw` | | Session | `forceRedraw(stdout)` — full UI repaint, recovers terminal drift. | none (pure Ink) → keep as-is |
| `/history` | | Session | Renders the TUI's **own** in-memory transcript (user+assistant) in a pager. `[N]` = char preview length. Note: deliberately does NOT call the worker `/history` (which only sees pre-process persisted turns). | none → read Argo transcript |
...
| `/logs` | | Session | Tail gateway log buffer in pager. | `gw.getLogTail` → Argo log file |
| `/reload` | | Tools | Re-read `~/.hermes/.env` into running gateway. | `reload.env` → Argo env reload |
...
| `/tools` | | Tools | `list` (worker) / `enable`/`disable <name…>` (built-in toolset or `mcp:tool`); change resets client history. | `tools.configure` → Argo tool registry |
...
| `/update` | | Info | Exits TUI with code 42 → Python wrapper execs `hermes update`. | local (Argo: own updater) |
...
| Command | Aliases | Cat | Flag | DOES | Argo port |
|---|---|---|---|---|---|
| `/goal` | | Session | | Set/pause/resume/clear/status a standing goal worked across turns. Returns `exec`/`send`. | **Maps directly to Argo kernel goals.** High value. |
| `/subgoal` | | Session | | Add/remove/clear criteria on the active goal. | Argo kernel goals. |
| `/commands` | | Info | gateway_only | Paginated browse of all commands+skills. | N/A (TUI uses `/help`) |
| `/snapshot` | `snap` | Session | cli_only | Create/restore/prune config+state snapshots. | Argo `~/.argo` state snapshot |
| `/curator` | | Tools | | Background skill maintenance (status/run/pin/archive…). | Optional — Argo skill maintenance |
| `/cron` | | Tools | cli_only | Manage scheduled tasks (list/add/edit/pause/run/remove). | Argo scheduler (future) |
| `/kanban` | | Tools | | Multi-profile collaboration board. | N/A early — multi-agent feature |
| `/bundles` | | Tools | | List skill bundles (alias `/<name>` = multiple skills). | Argo skill bundles (optional) |
| `/toolsets` | | Tools | cli_only | List available toolsets. | Argo tool registry list |
| `/config` | | Config | cli_only | Show current configuration. | Read Argo config |
| `/footer` | | Config | | Toggle gateway runtime-metadata footer. | optional |
| `/codex-runtime` | `codex_runtime` | Config | | Toggle codex app-server runtime for OpenAI models. | provider-specific; skip unless Argo supports |
...
| `/profile` | | Info | | Show active profile + home dir. | Argo profile/home |
| `/whoami` | | Info | | Show slash-command access level (admin/user). | N/A (single-user) |
| `/plugins` | | Tools | cli_only | List installed plugins. | N/A unless Argo has plugins |
| `/debug` | | Info | | Upload debug report (system info + logs), get links. | optional local diag |
| `/handoff` | | Session | cli_only | **Transfers the CLI session to a messaging platform's home channel** (`cli.py:_handle_handoff_command`): validates platform + home channel, refuses mid-turn, writes `handoff_state='pending'` to `state.db`, block-polls ≤60s for `completed`/`failed`, on success prints a `/resume` hint and exits the CLI. | **Argo N/A** — depends on gateway messaging platforms Argo doesn't have. |
| `/start` | | Session | gateway_only | Acknowledge platform start pings. | **Argo N/A** |
| `/topic` | | Session | gateway_only | Telegram DM topic sessions. | **Argo N/A** |
| `/approve` `/deny` | | Session | gateway_only | Approve/deny a pending dangerous command (messaging). | Argo handles approvals via kernel/overlay, not these |
| `/sethome` | `set-home` | Session | gateway_only | Mark this chat as the home channel. | **Argo N/A** |
| `/restart` | | Session | gateway_only | Gracefully restart the gateway. | **Argo N/A** |
| `/platforms` | `gateway` | Info | cli_only | Gateway/messaging platform status. | **Argo N/A** |
| `/platform` | | Info | gateway_only | Pause/resume/list a failing platform. | **Argo N/A** |

---

## 5. Argo port — priority guide
...
**Tier 3 (Argo-distinctive, kernel-backed):** `/goal`, `/subgoal`, `/agents`, `/rollback`, `/snapshot` — these map onto Argo's goal/approval kernel and `~/.argo` state and are where Argo can differentiate.

**Skip (gateway/messaging only):** `/handoff`, `/start`, `/topic`, `/approve`, `/deny`, `/sethome`, `/restart`, `/platforms`, `/platform`, `/commands`, `/kanban`.

**Key implementation note:** in Argo there is no `slash.exec`/`command.dispatch` round-trip. Build ONE in-process command table (Hermes's TS `SLASH_COMMANDS` shape — `{name, aliases, help, run(arg, ctx)}`). Each `run` calls Argo's sessions store / provider / tool registry / kernel directly. Preserve the `send` vs `prefill` vs `skill` distinction in `ctx` (enqueue-turn / fill-composer / inject-skill-prompt).
