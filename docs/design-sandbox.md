# Design — VANTA-SANDBOX (OS sandbox for tool execution)
...
## Goal
Opt-in OS-level isolation for `shell_cmd` and `run_code` so a tool the agent runs cannot read/write outside the project's scope or reach the network unless allowed. Defense-in-depth *under* the kernel verdict + VANTA-DANGEROUS-PATHS + VANTA-PERMISSIONS — those decide *whether* to run; the sandbox bounds *what a run can touch*.
...
OS-specific + security-critical; the profile must be exactly right. Build it fresh from this spec with the manual escape checks as the gate. Pairs with the shipped VANTA-DANGEROUS-PATHS (same DANGEROUS_DIRS) and VANTA-PERMISSIONS.
