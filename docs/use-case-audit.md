...
- Total Hermes capability areas surveyed: 67
...
- N/A (Vanta post-OpenClaw lineage): 1 (1%)
...
> Note on table vs summary reconciliation: counts above are over the 67 Hermes capabilities in the parity map data island. The map below omits ~9 off-thesis "forget" rows (Nous Portal, Web dashboard, isolated profiles, i18n, RL datagen, pairing codes, Bitwarden, computer-use desktop, plugin framework) and adds ~10 Vanta-distinctive capabilities with no Hermes equivalent (AUTO-ROUTER, auto-handoff, MEM-RELEVANCE, EF gates, operator task stack, OPERATOR-DASHBOARD, VISION-COMPARE, streaming, MCP-gated-serve, brain/selfhood); table icon counts therefore exceed the summary totals.

## Capability map

### Setup / Models

| Capability area | Hermes | Vanta | Status | Notes |
|---|---|---|---|---|
| Setup wizard | `hermes setup` | `vanta setup` first-run wizard | ✅ | |
| Doctor / status | `hermes doctor` | `vanta doctor` + kernel doctor, `vanta status` | ✅ | |
| Provider auth | Pooled creds, login/logout | env keys + OAuth: codex, claude-code (live-verified) | ✅ | Vanta also has subscription OAuth |
| Config show/edit/migrate | `hermes config` | `.env` + `upsertEnv` (merge, preserve) | 🟡 | Config-migrate folded into SELF-UPDATE |
...
| Model select | `hermes model` — CLI pulldown | TUI ModelPicker only | 🟡 | → MODEL-CMD (S, next) |
...
| Fallback chain on failure | `hermes fallback` — primary dies → next | none — dead provider fails the turn | ❌ | → MODEL-FALLBACK (next) |
| Local OpenAI-compat proxy | `hermes proxy` over OAuth | none | ❌ | → PROXY-ENDPOINT (next) |
...
| Webhook subscriptions | `hermes webhook` | Webhook listener + HMAC verify | ✅ | |
| Send to platform | `hermes send` | Gateway reply only | 🟡 | → CLI-DX-PACK |
...
| Cron / scheduled jobs | `hermes cron` | `vanta cron` + schedule runner | ✅ | |
...
| Capability area | Hermes | Vanta | Status | Notes |
|---|---|---|---|---|
| Skill search/install/manage | `hermes skills` | `write_skill`/`recall` + library auto-install | ✅ | |
...
| Auto-handoff / continuity | — | AUTO-HANDOFF (shipped) | ✅ | Vanta-distinctive — no Hermes equivalent |
| Memory relevance gating | — | MEM-RELEVANCE (shipped) | ✅ | Vanta-distinctive |
| Backup / import | `zip ~/.hermes` | `~/.vanta` git-init'd (free versioning) | 🟡 | → CLI-DX-PACK (vanta backup) |
...
| Capability area | Hermes | Vanta | Status | Notes |
|---|---|---|---|---|
| Action safety gating | None at the boundary | Rust kernel `assess()` — ENFORCED boundary | ✅ | Vanta-distinctive — Hermes has NO equivalent |
| Approval before risk | Advisory only | Kernel `ApprovalQueue` (persisted) | ✅ | Vanta-distinctive |
| Supply-chain audit | OSV, `hermes security audit` | none | ❌ | → DEP-AUDIT (next) |
...
| Capability area | Hermes | Vanta | Status | Notes |
|---|---|---|---|---|
| ACP server (Zed editor) | `hermes acp` — in-editor agent | none (MCP server only) | ❌ | → ACP-SERVE (next) |
| MCP client | Use servers | `mcp/client` + `mount.ts` | ✅ | |
| MCP server | Serve Vanta as an MCP server | `mcp/server.ts` — every call `assess()`-gated | ✅ | Vanta gates; Hermes doesn't |
...
| Capability area | Hermes | Vanta | Status | Notes |
|---|---|---|---|---|
| Update + rollback | `hermes update` — snapshot, autostash, dep/skill refresh | `install.sh`/`run.sh` self-bootstrap only | ❌ | → SELF-UPDATE (M, next) |
| Logs / debug | Events log | `events.jsonl` + kernel logs | ✅ | |
| Relaunch / service manager | External service | launchd user agent | ✅ | |
| Executive-function gates | None | Inhibit/set-shift/closure/research/self-monitor (shipped) | ✅ | Vanta-distinctive — no Hermes equivalent |
...
1. **SELF-UPDATE** — Safe self-updater with rollback. Hermes snapshots before mutating; Vanta has no equivalent. Medium, pebble.
...
5. **DEP-AUDIT** — Supply-chain audit (npm advisory + OSV). Hermes caught a PyPI worm; Vanta gates actions but never audits its own deps. Small, sand.
...
**Vanta's differentiation vs Hermes** (no Hermes equivalent): Rust `assess()` kernel, persistent `ApprovalQueue`, EF gate suite (inhibit/set-shift/closure/research-gate/self-monitor), auto-handoff, memory relevance gating, 5D/12-axis git-versioned brain, operator task stack + dashboard.
