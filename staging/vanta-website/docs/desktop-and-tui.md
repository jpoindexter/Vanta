...
### Context doctor contract

The full Vanta runtime should reserve `/doctor` for a read-only context health
check. It inventories always-loaded project instructions, reports their estimated
weight, finds duplicate or conflicting guidance, and proposes moving situational
procedures into skills. It must not silently delete instructions, change
permissions, or target a fixed reduction percentage. Cleanup requires a reviewed
diff; permission changes remain a separate decision.

The installable reference implementation lives in
`context-engineering-skills/`. This source snapshot does not contain the slash
command registry, so the documentation defines the merge contract rather than
claiming the built-in command is currently executable.
