...
export function KeyboardShortcuts(props: { open: boolean; onClose: () => void }) {
  ...
  const rows = [
    [`${command} N`, "New session"], [`${command} K`, "Command palette"], ["?", "Keyboard shortcuts"], ["Esc", "Close the active dialog"], ["Enter", "Send message"], ["Shift Enter", "Insert newline"], ["@", "Open file picker"], ["/", "Open quick actions"],
  ];
