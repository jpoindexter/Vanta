...
export function Composer(props: { value: string; busy: boolean; model?: string; attachments: string[]; onChange: (value: string) => void; onSubmit: (text: string) => void; onQueue: (text: string) => void; onFiles: (files: File[]) => void; onRemoveAttachment: (file: string) => void; onStop: () => void; onModel: () => void; onCommand: () => void }) {
  ...
  function addFiles(files: FileList | File[]) {
    ...
    if (next.length) props.onFiles(next);
  }
  function hasFiles(event: DragEvent<HTMLFormElement>) {
  ...
  return (
    <form
      className={`composer${draggingFiles ? " is-dragging-files" : ""}`}
      onSubmit={send}
      onDragEnter={(event) => { if (!hasFiles(event)) return; event.preventDefault(); dragDepth.current += 1; setDraggingFiles(true); }}
      onDragOver={(event) => { if (hasFiles(event)) event.preventDefault(); }}
      onDragLeave={(event) => { if (!hasFiles(event)) return; dragDepth.current -= 1; if (dragDepth.current <= 0) { dragDepth.current = 0; setDraggingFiles(false); } }}
      onDrop={(event) => { if (!hasFiles(event)) return; event.preventDefault(); dragDepth.current = 0; setDraggingFiles(false); addFiles(event.dataTransfer.files); }}
    >
      <label className="sr-only" htmlFor="vanta-composer">Message Vanta</label>
      <input ref={fileInput} className="file-input" type="file" multiple onChange={handleFileInput} aria-label="Attach files" />
      <textarea id="vanta-composer" value={props.value} onChange={(e) => props.onChange(e.target.value)} onKeyDown={(event) => keyDown(event, props, openFilePicker)} placeholder={props.busy ? "Queue the next instruction..." : "Ask Vanta to do something..."} />
      ...
      <div className="composer-footer">
        <div className="composer-context-controls"><button className="attach-button" type="button" title="Attach files" aria-label="Attach files" onClick={openFilePicker}><Paperclip size={16} /></button><button className="model-button" type="button" title="Change model" onClick={props.onModel}><Cpu size={15} /><span>{props.model ?? "Choose model"}</span></button></div>
...
function keyDown(event: KeyboardEvent<HTMLTextAreaElement>, props: Pick<Parameters<typeof Composer>[0], "value" | "onCommand">, onOpenFilePicker: () => void) {
