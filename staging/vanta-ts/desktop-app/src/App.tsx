    ...
    >
      ...
      <main className="workbench">
        {view === "work" ? <>
          ...
          <Composer value={convo.draft} busy={convo.busy} model={data.status?.model} attachments={attachments} onChange={convo.setDraft} onSubmit={(text) => { void convo.submit(withAttachments(text, attachments)); setAttachments([]); }} onQueue={convo.queue} onFiles={(files) => setAttachments((current) => appendAttachments(current, files))} onRemoveAttachment={(file) => setAttachments((current) => current.filter((entry) => entry !== file))} onStop={convo.stop} onModel={data.openModelPicker} onCommand={data.openPalette} />
...
function appendAttachments(current: string[], files: File[]): string[] {
  const next = files.map((file) => localFilePath(file)).filter(Boolean);
  return [...new Set([...current, ...next])];
}

function localFilePath(file: File): string {
  const path = (file as File & { path?: unknown }).path;
  return typeof path === "string" && path.trim() ? path : file.name;
}
