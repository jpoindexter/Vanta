...
The Work surface consumes the desktop's existing SSE channel. Text deltas render as a temporary assistant message and the five most recent tool/run events remain visible in the conversation; the final API result remains the saved conversation record. The signed-package live proof now exercises the real provider path. The paperclip and `@` open the native multi-file picker, and files can also be dropped onto the composer; selected files appear as removable context chips and submit through Vanta's existing `@file` syntax. `/` opens quick actions.
