...
- The work is production path — use `writing-plans` instead
...
- Search the web for candidate libraries (e.g. "python websocket streaming libraries 2025")
- Fetch and read the actual docs pages
- Check what's already installed in the project's environment (e.g. `pip show websockets | grep Version`)

For libraries without docs pages, clone and read their `README.md` / `examples/`. A docs MCP (if the user has one configured) is also a good source.
...
**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out with parallel subagents (the `delegate` tool):

```
Subagent 1 — Goal: Build 002a-pdf-parse-pdfjs: ...
Subagent 2 — Goal: Build 002b-pdf-parse-camelot: ...
```
