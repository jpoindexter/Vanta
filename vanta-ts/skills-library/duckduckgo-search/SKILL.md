...
Preferred when the built-in web search is unavailable or unsuitable (for example when a search API key is not set). Can also be used as a standalone search path when DuckDuckGo results are specifically desired.
...
Decision tree:
1. If `ddgs` CLI is installed, prefer running it in the shell
2. If `ddgs` CLI is missing, do not assume a Python runtime can import `ddgs`
...
- The shell and any code-execution runtime are separate environments
- A successful shell install does not guarantee a code-execution runtime can import `ddgs`
- Never assume third-party Python packages are preinstalled inside a code-execution runtime
...
Use the `ddgs` command in the shell when it exists. This is the preferred path because it avoids assuming a code-execution sandbox has the `ddgs` Python package installed.
...
Use the `DDGS` class in a Python runtime only after verifying that `ddgs` is installed there. Do not assume a code-execution runtime includes third-party packages by default.
...
DuckDuckGo returns titles, URLs, and snippets — not full page content. To get full page content, search first and then fetch the most relevant URL with a web-fetch tool, browser tools, or curl.
...
Then fetch the best URL with a web-fetch tool or another content-retrieval tool.

## Limitations

- **Rate limiting**: DuckDuckGo may throttle after many rapid requests. Add a short delay between searches if needed.
- **No content extraction**: `ddgs` returns snippets, not full page content. Use a web-fetch tool, browser tools, or curl for the full article/page.
- **Results quality**: Generally good but less configurable than dedicated search APIs.
...
- **Separate runtimes**: A successful `ddgs` install in the shell does not automatically mean a code-execution runtime can import it.
...
| CLI works but Python import fails | The shell and code-execution runtime are different environments | Keep using CLI, or separately prepare the Python runtime |
...
- **Do not assume a code-execution runtime can import `ddgs`**: `from ddgs import DDGS` may fail with `ModuleNotFoundError` unless that runtime was prepared separately.
...
Validated examples against `ddgs==9.11.2` semantics. Skill guidance treats CLI availability and Python import availability as separate concerns so the documented workflow matches actual runtime behavior.

## Attribution

Original skill by gamedevCloudy (MIT licensed).
