...
describe("lookAtCameraTool", () => {
  const saved = { p: process.env.VANTA_PROVIDER, k: process.env.OPENAI_API_KEY };
  afterEach(() => {
    if (saved.p === undefined) delete process.env.VANTA_PROVIDER; else process.env.VANTA_PROVIDER = saved.p;
  ...
  it("fails fast (no capture) without a model", async () => {
    process.env.VANTA_PROVIDER = "openai"; delete process.env.OPENAI_API_KEY;
