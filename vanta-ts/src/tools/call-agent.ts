...
function resolveCall(agent: string, prompt: string | undefined, model?: string): { error: string } | { inv: Invocation } {
  if (!prompt) return { error: "call_agent needs a prompt" };
  const inv = buildAgentInvocation(agent, prompt, model, process.env);
...
export const callAgentTool: Tool = {
  schema: {
    name: "call_agent",
    description:
      "Call ANOTHER AI coding-agent CLI non-interactively and return its answer — like opening a second agent in a new terminal. Auto-detects whatever is installed (claude, codex, gemini, cursor-agent, opencode out of the box; ANY other CLI/harness declared in ~/.vanta/agents.json). Call with no agent (or agent='list') to see what's available; otherwise pass {agent, prompt, model?}. The called agent runs in its own harness and default approval mode. Use to get a second model's take, delegate research/analysis, or cross-check.",
    parameters: {
      type: "object",
      properties: {
  ...
  async execute(raw, ctx: ToolContext): Promise<ToolResult> {
    ...
    const { agent, prompt, model } = parsed.data;

    if (!agent || agent === "list") return listAgents();

    const resolved = resolveCall(agent, prompt, model);
