...
export const driveCreateTool: Tool = {
  ...
  async execute(raw, ctx) {
    ...
    const { body, contentType } = buildMultipartBody({ name }, content, mimeType ?? DEFAULT_MIME);
    try {
      const r = await googleFetch(
        buildUrl(UPLOAD_URL, { uploadType: "multipart" }),
        { method: "POST", headers: { "Content-Type": contentType }, body },
      );
      if (!r.ok) return { ok: false, output: `drive_create failed: HTTP ${r.status}` };
      const json = UploadResponse.safeParse(await r.json());
      if (!json.success) return { ok: false, output: "drive_create: unexpected response shape" };
  ...
  async execute(raw, ctx) {
    ...
    try {
      const r = await googleFetch(
        ...
        { method: "PATCH", headers: { "Content-Type": mimeType ?? DEFAULT_MIME }, body: content },
      );
      if (!r.ok) return { ok: false, output: `drive_update failed: HTTP ${r.status}` };
