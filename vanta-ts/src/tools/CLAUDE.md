...
- Tool count: **121 built-in** in `ALL_TOOLS`, **125 registered** (+ factory `mount_mcp` + `tool_search` + `mcp_auth`) — real-counted 2026-06-24 via `buildRegistry().schemas()` (added `call_agent`). Update both numbers after a registry change.
