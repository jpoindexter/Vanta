...
- Tool count: **119 built-in** in `ALL_TOOLS`, **122 registered** (+ factory `mount_mcp` + `tool_search` + `mcp_auth`) — real-counted 2026-06-21 via `buildRegistry().schemas()` (added `terminal_capture` + `voice_input`). Update both numbers after a registry change.
