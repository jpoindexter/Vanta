...
- Tool count: **120 built-in** in `ALL_TOOLS`, **124 registered** (+ factory `mount_mcp` + `tool_search` + `mcp_auth`) — real-counted 2026-06-23 via `buildRegistry().schemas()` (added `google_auth`). Update both numbers after a registry change.
