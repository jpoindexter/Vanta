...
- Tool count: **148 built-in** in `ALL_TOOLS`, **152 registered** (+ factory `mount_mcp` + `tool_search` + `mcp_auth` + `run_pipeline`) — real-counted 2026-08-20. Update both numbers after a registry change.
...
- `shell-cmd.ts`: approved commands run through `sandbox/run.ts` when `VANTA_SANDBOX=1` or shell-only `VANTA_SHELL_SANDBOX=1`; background tasks retain that sandbox wrapper and own its cleanup. Listening servers require `VANTA_SANDBOX_NET=1`, which is already set by the default macOS sandbox policy.
