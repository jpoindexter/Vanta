...
function roadmapHealth(data: Roadmap): string {
  ...
  const parkedLine = PARKED_REASON
    ...
    .join(" · ");
  const completeLine = summary.complete
    ? `No open roadmap cards remain${summary.terminalParkedTotal ? `; ${summary.terminalParkedTotal} terminally parked.` : "."}`
    : `${summary.openTotal} open card${summary.openTotal === 1 ? "" : "s"} remain${summary.openTotal === 1 ? "s" : ""}; ${summary.terminalParkedTotal} terminally parked.`;
