...
// (the classifier mode that auto-approves safe reads plus routine file edits
// from the kernel `ask` tier — see permissions/auto-mode.ts and modes/
// permission-mode.ts). Turning auto mode on must
...
// the prompt for safe reads and ordinary edits; bypass skips the prompt for the
// WHOLE ask tier with no exceptions.
//
...
//   3. Auto only auto-approves safe reads and bounded file tools — a risky or
//      destructive action the classifier flags unsafe still PROMPTS/BLOCKS.
 ...
 * it spells out what auto-approves (safe reads and ordinary file edits), what
 * STILL prompts (destructive / consequential actions), reaffirms that the kernel
...
export function buildAutoModeExplanation(): string {
  return [
    ...
    "",
    "Auto mode AUTO-APPROVES safe reads and routine file edits after the kernel",
    "checks them — reads, searches, discovery, state inspection, and ordinary file",
    "changes — so autonomous work does not stop for repetitive confirmations.",
    "",
    "What STILL PROMPTS (auto never auto-approves these):",
    "  • destructive or irreversible actions",
    "  • consequential shell, network, account, or external-service actions",
    ...
    "in auto mode. Auto only skips prompts for safe reads and bounded file edits.",
    "",
