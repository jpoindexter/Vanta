...
// Pure parse/build/allowlist helpers for the Microsoft Teams adapter (the adapter class +
// live Bot Connector transport live in `teams.ts`). Everything here is offline-unit-tested
// and re-exported from `teams.ts`, so the public module path (`./teams.js`) is unchanged.
//
...
// on poll (see `parseServiceUrls`) and sends the reply to that recorded URI.
...
export const TEAMS_TEXT_LIMIT = 28000;
