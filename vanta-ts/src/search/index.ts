...
export function resolveSearchProvider(env: NodeJS.ProcessEnv): SearchProvider {
  ...
  switch (provider) {
    ...
    case "searxng": {
      ...
      if (!baseUrl) {
        throw new Error(
          "VANTA_SEARCH_URL is required for searxng. Set it in vanta-ts/.env (e.g. http://localhost:8080).",
        );
      }
    ...
    case "serpapi": {
      ...
      if (!apiKey) {
        throw new Error(
          "SERPAPI_KEY is required for serpapi. Set it in vanta-ts/.env, or use VANTA_SEARCH_PROVIDER=ddg for keyless search.",
        );
      }
    ...
    case "brave": {
      ...
      if (!apiKey) {
        throw new Error(
          "BRAVE_KEY is required for brave. Set it in vanta-ts/.env, or use VANTA_SEARCH_PROVIDER=ddg for keyless search.",
        );
      }
