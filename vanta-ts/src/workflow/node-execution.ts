...
import type { ReviewPacket } from "./review-contract.js";
...
export function validateNodeEvidence(node: WorkflowNode, outcome: GraphAgentOutcome): void {
  const allowed = new Set(node.type === "agent" || node.type === "review" ? node.evidence ?? [] : ["receipt"]);
...
export function nodeResult(node: WorkflowNode, status: WorkflowNodeStatus, output = "", details: { outputs?: Record<string, GraphTypedOutput>; handoffs?: GraphHandoffReceipt[]; review?: ReviewPacket } = {}): GraphNodeResult {
  return { nodeId: node.id, type: node.type, status, output, outputs: details.outputs ?? {}, handoffs: details.handoffs ?? [], review: details.review };
}

export function describeNodeAction(node: WorkflowNode, inputs: Record<string, unknown> = {}): string {
  if (node.type === "agent" || node.type === "review") return `workflow ${node.type} node ${node.id}: ${node.instruction}`;
