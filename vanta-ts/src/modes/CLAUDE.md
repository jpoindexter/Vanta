...
- `permission-mode.ts`: `PermissionMode` (`default`/`acceptEdits`/`auto`/`fullAccess`) — permission authority beneath the kernel. Auto includes routine file edits as well as classifier-approved reads; kernel blocks remain immovable.
