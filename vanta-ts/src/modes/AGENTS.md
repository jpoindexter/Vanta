...
- `permission-mode.ts` is the authority tier (`default`/`acceptEdits`/`auto`/`fullAccess`), not an operator-mode skill. `operating-mode.ts` adds enforced `plan` and the Manual → Accept edits → Plan → Auto cycle shared by TUI, CLI, and Desktop. Accept edits clears routine file confirmations; Auto includes those edits plus its safe-action classifier. These edit the gate but never bypass kernel `block`.
