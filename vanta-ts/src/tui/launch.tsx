...
/**
 * Launch the Ink TUI (vendored hermes-ink fork). Prepares the run (kernel up,
 * provider, goals, system prompt), runs session-start curation, then renders
 * the React app and waits for exit. The App's own <AlternateScreen> component
 * handles the alt-screen buffer, mouse-wheel tracking, and unmount cleanup —
 * no render option needed.
 */
export async function runTui(repoRoot: string): Promise<void> {
  const setup = await prepareRun(repoRoot, "interactive session");
  await maybeCurate();
  const instance = renderSync(<App setup={setup} repoRoot={repoRoot} />);
