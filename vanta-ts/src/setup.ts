...
// The pure `.env` merge/write seam lives in `setup-env.ts` (re-exported below).

// Re-exported so importers + tests keep the `./setup.js` path for the env helpers.
export { envPath, upsertEnv, removeEnvKeys, upsertEnvMigratingLegacy, buildEnvUpdates, setEnv } from "./setup-env.js";
