...
describe("integration catalog", () => {
  it("reports every requested integration without calling an unconfigured service ready", async () => {
    const rows = await readIntegrationCatalog(root, {}, {
      googleAuthorized: async () => false,
      mcpRecords: emptyRecords,
      receipts: emptyReceipts,
      commandAvailable: () => false,
    });
    expect(rows.map((row) => row.id)).toEqual(["trello", "dropbox", "box", "google-drive", "atlassian-rovo", "slack", "telegram", "buzz"]);
    ...
    expect(rows.find((row) => row.id === "buzz")).toMatchObject({ state: "needs_setup", actions: ["configure"] });
  });
  ...
  it("marks Buzz ready only after binaries, identity, and a live relay test", async () => {
    const rows = await readIntegrationCatalog(root, { BUZZ_PRIVATE_KEY: "nsec-test" }, {
      googleAuthorized: async () => false,
      mcpRecords: emptyRecords,
      ...
      receipts: async () => [{
        ...
        action: "test",
        outcome: "passed",
        detail: "bounded relay read",
      }],
    });
    expect(rows.find((row) => row.id === "buzz")).toMatchObject({
      state: "ready",
