...
describe("/setup", () => {
  ...
  it("gives one exact Telegram setup command when unconfigured", async () => {
    ...
    expect(result.output).toContain("Token entry runs from the shell command above");
  ...
  it("keeps the direct Telegram command inside the interactive surface", async () => {
    const result = await createSetupCommand({ status: async () => ({ state: "needs_repair", title: "Telegram needs repair.", detail: "Telegram token rejected", action: { id: "configure", label: "Repair Telegram setup", command: "vanta setup messaging telegram" } }) })("telegram", ctx());
