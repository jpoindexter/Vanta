...
function hasProof(text: string): boolean {
  return text.includes("✓") || text.includes("Result:") || text.includes("Output:") || text.includes("tool_result");
}

export function selfEvalResponse(text: string): string[] {
  const flags: string[] = [];
  if (UNVERIFIED_COMMITMENT_RE.test(text) && !hasProof(text))
    flags.push("unverified commitment (said will, no tool proof)");
  if (COMPLETION_CLAIM_RE.test(text) && !hasProof(text) && !text.includes("DONE:"))
