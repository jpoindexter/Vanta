...
import { bgtasks } from "./bgtasks-cmd.js";
...
import { reloadSkills } from "./reload-skills-cmd.js";
...
import { autonomy } from "./autonomy-cmd.js";
...
const home: SlashHandler = async (_arg, ctx) => ({
  output: await buildOperatorHome({
    dataDir: ctx.dataDir,
    env: ctx.env,
    toolNames: ctx.setup.registry.schemas().map((schema) => schema.name),
  }),
});
...
export const HANDLERS: Record<string, SlashHandler> = {
  help, home, blueprint, "what-can-i-do": whatCanIDo, "diagnose-crash": diagnoseCrash, "spec-to-app": specToApp, autonomy, exit, quit: exit, init, "init-verifiers": initVerifiers, clear, new: clear, reset: clear, attachments, history,
