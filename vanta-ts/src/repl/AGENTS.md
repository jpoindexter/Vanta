...
- `/restart` must save the current session before exiting and write only the session ID plus timestamp to `restart-handoff.ts`; if either persistence step fails, cancel the reload instead of discarding context.
...
- `/prompt list|show|use|reset` switches only the marked prompt-preset block in the live system message, preserving goals, plan mode, and the immutable base safety contract.
