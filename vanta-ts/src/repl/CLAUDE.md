...
- `restart-cmd.ts` owns `/restart`; `restart-handoff.ts` is its read-once, five-minute, permission-restricted session-ID bridge across the run.sh process relaunch. Transcript content stays in `sessions/store.ts`.
