...
describe("resolveToolBudget", () => {
  ...
  it("keeps enough default headroom for a multi-step build", () => {
    expect(DEFAULT_TOOL_BUDGET).toBeGreaterThanOrEqual(100);
    expect(CORRECTION_TOOL_BUDGET).toBeGreaterThanOrEqual(50);
  });
...
describe("two-phase tool budget", () => {
  it("enters closure before the hard ceiling without raising it", () => {
    expect(effectiveToolBudget(false, DEFAULT_TOOL_BUDGET)).toBe(DEFAULT_TOOL_BUDGET);
    const threshold = DEFAULT_TOOL_BUDGET - DEFAULT_TOOL_CLOSURE_RESERVE;
    expect(toolClosureThreshold(false, DEFAULT_TOOL_BUDGET)).toBe(threshold);
    expect(shouldEnterToolClosure(threshold - 1, false, DEFAULT_TOOL_BUDGET)).toBe(false);
    expect(shouldEnterToolClosure(threshold, false, DEFAULT_TOOL_BUDGET)).toBe(true);
    expect(shouldHaltForToolBudget(threshold, false, DEFAULT_TOOL_BUDGET)).toBe(false);
  });
  ...
  it("uses a smaller but still two-phase correction budget", () => {
    ...
    const threshold = CORRECTION_TOOL_BUDGET - DEFAULT_TOOL_CLOSURE_RESERVE;
    expect(toolClosureThreshold(true, DEFAULT_TOOL_BUDGET)).toBe(threshold);
    expect(shouldEnterToolClosure(threshold, true, DEFAULT_TOOL_BUDGET)).toBe(true);
    expect(shouldHaltForToolBudget(threshold, true, DEFAULT_TOOL_BUDGET)).toBe(false);
