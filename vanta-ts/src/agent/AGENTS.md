...
- Auto mode keeps the general bounded tool ceiling when the operator corrects or re-asks; the tighter correction leash is for Manual and Accept edits. Explicit unfinished text auto-continues within the generic cap. A successfully observed open checklist is stronger state and keeps the turn alive until it closes, a real user decision is required, Auto-continue is explicitly disabled, or the existing hard turn boundary is reached. Every forced terminal outcome is appended to the transcript with its reason.
