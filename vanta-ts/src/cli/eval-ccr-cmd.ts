...
The whole-retrieve rate is the deciding signal. Vanta's CCR makes re-expansion **optional** (an \`original_id\` in a footer), unlike a forced full-retrieve — the pattern that makes CCR net-negative. A **low** rate means the compressed/skeletoned view sufficed and CCR saved tokens; a **high** rate means the agent pulls originals back whole and pays the double-context tax.
