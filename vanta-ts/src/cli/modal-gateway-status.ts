...
export type GatewayStatusReport = {
  ...
  telegram: { endpoint?: string; registeredAt?: string; token: TelegramTokenState; webhookSecret: "present" | "missing" };
...
export function statusNextLines(cfg: GatewayConfig, state: GatewayState, receipt: GatewayReceipt, env: NodeJS.ProcessEnv): string[] {
  ...
  if (token === "invalid-format") lines.push("next: replace VANTA_TELEGRAM_TOKEN with a valid BotFather token");
