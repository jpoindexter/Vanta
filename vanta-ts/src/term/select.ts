...
/** Pure: the rendered menu frame with `active` highlighted (❯). */
export function renderMenu(title: string, options: string[], active: number, canBack: boolean): string {
  const rows = options.map((o, i) => (i === active ? `  ❯ ${o}` : `    ${o}`));
  const hint = `  ↑/↓ move · Enter select${canBack ? " · Esc back" : ""} · Ctrl+C quit`;
  return [`  ${title}`, ...rows, hint].join("\n");
}
...
export function select(title: string, options: string[], opts: SelectOpts = {}): Promise<number> {
  ...
  return new Promise((resolve) => {
    ...
    const draw = () => {
      ...
      const frame = renderMenu(title, options, active, canBack);
      output.write(frame + "\n");
      prev = frame.split("\n").length;
    };
