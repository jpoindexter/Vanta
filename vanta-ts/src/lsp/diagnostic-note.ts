...
// OPT-IN: getDiagnostics runs a full semantic TS build, so straddling a write
// costs TWO of them. That's too much latency to impose on every .ts edit by
// default — enable per-need with VANTA_DIAGNOSTIC_DELTA=1.
function isEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.VANTA_DIAGNOSTIC_DELTA === "1";
}
...
export async function beginDiagnosticDelta(
  ...
  if (!isEnabled(process.env) || !isEligible(abs)) return async () => "";
  let before: Diag[];
  try {
