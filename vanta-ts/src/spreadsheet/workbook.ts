...
export async function inspectWorkbook(path: string, options: InspectOptions = {}) {
  ...
  const sheets = workbook.worksheets.map((sheet) => ({ name: sheet.name, rows: sheet.actualRowCount, columns: sheet.actualColumnCount }));
...
export async function previewWorkbookPlan(path: string, plan: WorkbookPlan): Promise<{ lines: string[]; touched: string[] }> {
  ...
  for (const change of plan.changes) {
    touched.push(change.kind === "set" || change.kind === "formula" ? `${change.sheet}!${change.cell}` : change.sheet);
...
function applyChanges(workbook: ExcelJS.Workbook, plan: WorkbookPlan): void {
  for (const change of plan.changes) {
