...
- Desktop access mode is project-scoped through `/api/access-mode`: Manual prompts, Accept edits auto-confirms project file operations, Plan is read-only, Auto applies the safe-action classifier, and Full access clears Ask prompts without bypassing kernel or explicit Block decisions.
