...
// COFOUNDER-DEPARTMENT-UNIT (pure model) — a department is a first-class org unit:
// ONE durable entity that owns a worker roster, a scoped budget, a standing goal
// subset, and a default skill bundle. It binds (does not duplicate) the existing
// primitives: workerIds reference team/* workers, budgetScope reuses budget/*
// (`dept:<id>`), goalIds reference the kernel goal ledger. The model + status views
// are pure/injectable (status is computed against injected budget/worker/goal
// snapshots); persistence lives in department.ts, which re-exports this model.
