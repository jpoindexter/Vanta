...
describe("classifyCompartment", () => {
  it("classifies the kernel + factory + manifesto as skeleton", () => {
    ...
    expect(classifyCompartment("vanta-ts/src/factory/run.ts")).toBe("skeleton");
    expect(classifyCompartment("vanta-ts/src/factory/compartments.ts")).toBe("skeleton");
    expect(classifyCompartment("MANIFESTO.md")).toBe("skeleton");
  });

  it("classifies the agent loop + its lifeline as brainstem", () => {
    expect(classifyCompartment("vanta-ts/src/agent.ts")).toBe("brainstem");
    expect(classifyCompartment("vanta-ts/src/providers/openai.ts")).toBe("brainstem");
    expect(classifyCompartment("vanta-ts/src/prompt.ts")).toBe("brainstem");
    expect(classifyCompartment("vanta-ts/src/context.ts")).toBe("brainstem");
    expect(classifyCompartment("vanta-ts/src/session.ts")).toBe("brainstem");
    expect(classifyCompartment("vanta-ts/src/safety-client.ts")).toBe("brainstem");
  });

  it("classifies skills as reflexes", () => {
    expect(classifyCompartment("vanta-ts/src/skills/recall.ts")).toBe("reflexes");
    expect(classifyCompartment("skills-library/debug/SKILL.md")).toBe("reflexes");
  });

  it("classifies brain + memory as memory", () => {
    expect(classifyCompartment("vanta-ts/src/brain/store.ts")).toBe("memory");
    expect(classifyCompartment("vanta-ts/src/memory/store.ts")).toBe("memory");
  });

  it("classifies tools + other app code as limbs", () => {
    expect(classifyCompartment("vanta-ts/src/tools/web-search.ts")).toBe("limbs");
    expect(classifyCompartment("vanta-ts/src/tui/transcript.tsx")).toBe("limbs");
    expect(classifyCompartment("vanta-ts/src/status.ts")).toBe("limbs");
    expect(classifyCompartment("vanta-ts/src/gateway/run.ts")).toBe("limbs");
  });
});
...
describe("autonomyCapForFiles", () => {
  it("a pure-limbs slice permits L5", () => {
    const cap = autonomyCapForFiles(["vanta-ts/src/tools/a.ts", "vanta-ts/src/tools/a.test.ts"]);
  ...
  it("takes the most restrictive compartment across a mixed slice", () => {
    const cap = autonomyCapForFiles(["vanta-ts/src/tools/a.ts", "vanta-ts/src/agent.ts"]);
    expect(cap.maxLevel).toBe(2);
    expect(cap.compartment).toBe("brainstem");
  });

  it("a skeleton-touching slice caps at 0", () => {
    const cap = autonomyCapForFiles(["vanta-ts/src/tools/a.ts", "src/safety.rs"]);
