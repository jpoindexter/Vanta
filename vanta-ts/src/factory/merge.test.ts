...
const armed = {
  touchedFiles: ["vanta-ts/src/tools/web-search.ts", "vanta-ts/src/tools/web-search.test.ts"],
  diffLineCount: 40,
  allowMerge: true,
...
describe("assessMergeRisk", () => {
  ...
  it("refuses when a touched file leaves the limbs/reflexes/memory tiers", () => {
    const d = assessMergeRisk({ ...armed, touchedFiles: [...armed.touchedFiles, "vanta-ts/src/agent.ts"] });
    expect(d.merge).toBe(false);
    expect(d.reason).toMatch(/compartment|brainstem/i);
  });

  it("refuses when a dependency / lockfile changed", () => {
    expect(assessMergeRisk({ ...armed, touchedFiles: [...armed.touchedFiles, "vanta-ts/package.json"] }).merge).toBe(false);
    expect(assessMergeRisk({ ...armed, touchedFiles: [...armed.touchedFiles, "vanta-ts/package-lock.json"] }).merge).toBe(false);
  });

  it("refuses when env / config / migration files changed", () => {
    expect(assessMergeRisk({ ...armed, touchedFiles: [".env.example"] }).merge).toBe(false);
    expect(assessMergeRisk({ ...armed, touchedFiles: ["vanta-ts/vitest.config.ts"] }).merge).toBe(false);
  ...
  it("refuses when too many files changed", () => {
    const many = Array.from({ length: MAX_MERGE_FILES + 1 }, (_, i) => `vanta-ts/src/tools/t${i}.ts`);
