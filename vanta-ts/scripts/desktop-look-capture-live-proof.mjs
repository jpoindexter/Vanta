...
const captureOnly = process.env.VANTA_DESKTOP_LIVE_CAPTURE_ONLY === "1";
const captureMode = captureOnly ? "screen" : "screen";
...
try {
  ...
  await page.getByText(`LOOK · ${captureMode}`, { exact: true }).waitFor();
  await page.locator("#vanta-look-live-marker").evaluate((node) => node.remove());

  if (captureOnly) {
    console.log(JSON.stringify({ desktopLookLive: true, packaged: true, mode: captureMode, captured: true, providerCalled: false, persisted: false }));
  } else {
    let responseBody;
