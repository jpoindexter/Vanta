...
const session = `vanta-auto-compaction-proof-${process.pid}`;
const home = await mkdtemp(resolve(tmpdir(), "vanta-auto-compaction-proof-"));
...
const server = createServer(async (request, response) => {
  if (request.url === "/v1/models") {
    return json(response, { object: "list", data: [{ id: "auto-compaction-proof", object: "model" }] });
  }
  if (request.url !== "/v1/chat/completions") return void response.writeHead(404).end();
  const body = JSON.parse(await readBody(request));
  if (body.stream === true) {
    streamedTurns += 1;
    return streamText(response, `AUTO_COMPACTION_TURN_${streamedTurns}`);
  }
  summaryRequests += 1;
  await new Promise((resolveWait) => setTimeout(resolveWait, 900));
  json(response, {
    id: "auto-compaction-summary",
    object: "chat.completion",
    created: 1,
    model: "auto-compaction-proof",
    choices: [{
      index: 0,
      message: { role: "assistant", content: "Earlier automatic proof turns completed." },
      finish_reason: "stop",
    }],
...
try {
  await tmux("new-session", "-d", "-s", session, "-x", "100", "-y", "32", shellCommand({
    ...
    VANTA_MODEL: "auto-compaction-proof",
    VANTA_CONTEXT_WINDOW: "1000",
    VANTA_AUTO_COMPACT_THRESHOLD: "0.01",
  ...
  for (let index = 1; index <= 5; index += 1) {
    await send(`Retain automatic compaction proof history turn ${index}.`);
    await waitForPane(`AUTO_COMPACTION_TURN_${index}`);
  }

  await send("Trigger the automatic compaction proof now.");
  ...
  await waitForPane("AUTO_COMPACTION_TURN_6");
  const final = await capture();
  assert.match(final, /Ask Vanta anything/);
  assert.equal(streamedTurns, 6);
  assert.ok(summaryRequests >= 1, "automatic threshold must issue a summary request");
  console.log("tui-auto-compaction-meter: PASS");
  console.log("executed: project launcher → six real TUI turns → automatic threshold → live square-cell 25% milestone → resumed provider turn");
} finally {
