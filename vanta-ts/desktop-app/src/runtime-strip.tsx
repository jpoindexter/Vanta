...
import { ChevronDown, Cpu, Gauge, HardDrive, ListTree, Server, ShieldCheck, X } from "lucide-react";
import type { DesktopRuntime, RuntimeHostSnapshot } from "./types.js";

export function RuntimeStrip(props: { runtime: DesktopRuntime; onSelect: (hostId: string) => Promise<void> }) {
...
export function RuntimeDetail(props: {
  ...
  const pressure = pressurePercent(props.selected);
  return <section id="runtime-context-panel" className="runtime-context-panel" role="dialog" aria-label="Runtime details" aria-modal="false">
    ...
    <div className="runtime-detail-grid">
      <RuntimeFact label="Transport" value={transportLabel(props.selected)} tone={props.selected.transport === "reachable" ? "good" : "bad"} />
      <RuntimeFact label="Kernel boundary" value={props.selected.kernel === "ready" ? "Ready" : props.selected.kernel === "not_ready" ? "Not ready" : "Unknown"} tone={props.selected.kernel === "ready" ? "good" : "bad"} />
      <RuntimeFact label="Lifecycle" value={statusLabel(props.selected.status)} />
      <RuntimeFact label="Engine" value={props.selected.engine.id ?? "Idle"} />
      <RuntimeFact label="Memory pressure" value={`${pressure}%`} />
      <RuntimeFact label="Throughput" value={formatThroughput(props.selected.resources.throughputPerSecond)} />
      <RuntimeFact label="Queued turns" value={String(props.selected.queueDepth)} />
      <RuntimeFact label="Observed" value={props.selected.stale ? "Stale" : "Current"} tone={props.selected.stale ? "bad" : "good"} />
    </div>
