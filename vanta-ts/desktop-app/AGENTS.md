...
- Keep `src/App.tsx` as the shell/composition layer. Put API calls in `src/api.ts`, state hooks in `src/state.ts`, chat/sidebar/composer in `src/chat.tsx`, rail panels in `src/rail.tsx`, and overlays in `src/overlays.tsx`.
