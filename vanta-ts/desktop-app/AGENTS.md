...
- Native file and folder attachments cross the context-isolated preload bridge. Convert renderer `File` objects with Electron `webUtils`, expand and filter paths in `electron/dropped-paths.mjs`, and keep the renderer unaware of Node filesystem APIs. Preserve top-level grouping metadata: a folder renders as one icon-only composer chip while its filtered files remain available to submission.
