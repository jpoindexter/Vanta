...
| `hooks/` | VANTA-HOOK-TYPES — `.vanta/hooks.json` schema + execution. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; shared controls: `timeoutMs`, `once`, `statusMessage`. Agent hooks are dependency-injected around tool dispatch to avoid agent-loop import cycles. |
...
- **Current source counts beat historical session counts.** As of the 2026-06-18 context sync, `ALL_TOOLS` has 88 built-in tools (90 registered with factory `mount_mcp`/`tool_search`) and `SLASH_COMMANDS.length` reports 99 commands. Latest full verify: 471 TS test files / 3687 tests. Older counts in session history are milestones, not current truth.
