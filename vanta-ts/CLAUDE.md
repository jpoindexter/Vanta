...
**`MemoryStore` persistence port** (`store/memory-store.ts`, `resolveMemoryStore`, `VANTA_MEMORY_STORE`) — backend-agnostic `read`/`write`/`append`/`list`/`remove`/`ensure`/`commit` keyed by a path RELATIVE to ~/.vanta (e.g. `"world.jsonl"`, `"memories/5.md"`). ~34 data stores migrated to it (PORT-MEMORY-STORE shipped). Enforced by the `home-store-direct` fitness rule: persist via `resolveMemoryStore`, never `store/home.ts`. The 27 remaining direct importers are genuinely fs-bound (sync API, file-mode 0600, raw absolute paths for external processes, absolute-path public exports, dir-ops) and are **grandfathered shrink-only** in `lint/boundaries.ts` — never add to that list; new direct imports hard-fail. `slugifySkillName` (pure) lives in `store/slug.ts`.
