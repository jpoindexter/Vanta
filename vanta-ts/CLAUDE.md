...
| `tools/index.ts` | `buildRegistry({exclude?})` — registers the 89 `ALL_TOOLS` (91 with factory-built `mount_mcp`/`tool_search`; workers exclude recursive tools such as `delegate` as needed); `roadmap_add` files new cards, `roadmap_move` changes status |
...
| `skills-library/` | **43** shipped skills at `vanta-ts/skills-library/<slug>/SKILL.md`: ~10 ported from references + `agent-orchestration-workflows`, `build-mcp-server`, `vanta-port-adapter`, the **14 `nd-*` executive-function skills**, the **9 Boris-Cherny build-catalog skills** (`standing-loops`, `keep-green`, `prod-watch`, `cluster-feedback`, `hill-climb`, `agent-fanout`, `parallel-verify-workflows`, `ship-preflight`, `agentic-build-strategy`), and the **2 build/verify disciplines** (`source-grounded` — cite official docs over training data; `doubt-gate` — in-flight adversarial cross-examination). Add a `<slug>/SKILL.md` dir to grow the bundle; it auto-installs on next session |
...
- **Current source counts beat historical session counts.** As of the 2026-06-20 context sync, `ALL_TOOLS` has 94 built-in tools (97 registered with factory `mount_mcp`/`tool_search`/`mcp_auth`) and `SLASH_COMMANDS.length` reports 99 commands. Latest full verify: 562 TS test files / 4490 tests. Older counts in session history are milestones, not current truth.
