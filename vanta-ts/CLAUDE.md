...
| `ui/theme.tsx` | `ThemeProvider`/`useTheme` over a SINGLE fixed palette from `term/theme.ts` (`THEME`/`resolveTheme()`). The multi-theme selection + terminal-bg detection (`VANTA_THEME`, COLORFGBG/OSC, `/theme`) were removed as unused; colours still live in one place (the token struct) so the UI isn't hardcoded |
...
| `gateway/webhook.ts` | v1 E3 — pure `verifyGithubSignature` (HMAC sha256, constant-time) + `resolveDeliver` over the `DELIVERY_TARGETS` registry (local/file/telegram; add a channel like `slack:` = one registry entry, not a switch edit) + `startWebhookServer` (POST, HMAC-gated, 200-fast). `VANTA_WEBHOOK_PORT`/`_SECRET`/`_PROMPT`/`_DELIVER` |
...
| `gateway/platforms/base.ts` | v1 E2 — `PlatformAdapter` **port** (`connect`/`disconnect`/`send`/`poll`) + `InboundMessage`/`OutboundMessage`. One adapter per messaging platform |
| `gateway/platforms/index.ts` | `resolvePlatformAdapter(env)` — the messaging factory (mirrors `resolveProvider`): explicit `VANTA_MESSAGING_PLATFORM` or inferred from configured creds (telegram/signal/imessage). Consumers (gateway) use this, never a concrete adapter (the `messaging-platform-port` fitness rule enforces it). Adding a platform = a new adapter + one case |
...
| `a2a/{types,local}.ts` | Phase 6 — `A2ATransport` **port** (`send`) in types.ts; `A2ABus` (`makeMessage`) is the in-process adapter (`implements A2ATransport`). A networked HTTP/Google-A2A transport drops in as another `A2ATransport` impl; callers depend on the port (mirrors mcp/client.ts `Transport`) |
...
| `sessions/{interface,index,store}.ts` | **`SessionStore` port** (`sessions/interface.ts`) + `resolveSessionStore(env)` (`sessions/index.ts`, `VANTA_SESSION_STORE=fs`) + the fs-JSON adapter `fsSessionStore` (`store.ts`): `saveSession`/`loadSession`/`forkSession`/`deleteSession`/`listSessions`. Consumers use `resolveSessionStore()` from `sessions/index.js` — never `sessions/store` directly (the `session-store-port` fitness rule enforces it). `newSessionId` is a pure helper re-exported from index. JSON files `~/.vanta/sessions/<id>.json`, zod-validated. `vanta sessions`/`resume`/`--resume`/`--fork-session` |
...
**`MemoryStore` persistence port** (`store/memory-store.ts`, `resolveMemoryStore`, `VANTA_MEMORY_STORE`) — backend-agnostic `read`/`write`/`append`/`list`/`remove`/`ensure`/`commit` keyed by a path RELATIVE to ~/.vanta (e.g. `"world.jsonl"`, `"memories/5.md"`). ~36 data stores migrated to it (PORT-MEMORY-STORE shipped). Enforced by the `home-store-direct` fitness rule: persist via `resolveMemoryStore`, never `store/home.ts`. The 25 remaining direct importers are genuinely fs-bound (sync API, file-mode 0600, raw absolute paths for external processes, absolute-path public exports, dir-ops) and are **grandfathered shrink-only** in `lint/boundaries.ts` — never add to that list; new direct imports hard-fail. `slugifySkillName` (pure) lives in `store/slug.ts`.
...
- **Current source counts beat historical session counts.** As of the 2026-06-16 context sync, `ALL_TOOLS` has 84 built-in tools (86 registered with factory `mount_mcp`/`tool_search`) and `SLASH_COMMANDS.length` reports 97 commands. Older counts in session history are milestones, not current truth.
