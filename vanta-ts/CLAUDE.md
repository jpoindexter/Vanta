...
| `hooks/` | VANTA-HOOK-TYPES + VANTA-HOOK-AGENT-TYPE + VANTA-HOOK-EVENTS (building) — `.vanta/hooks.json` 30-event schema + execution. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; shared controls: `timeoutMs`, `once`, `statusMessage`. Agent hooks are dependency-injected for live tool/prompt/stop/session events to avoid agent-loop import cycles. |
...
- **Current source counts beat historical session counts.** As of the 2026-06-18 context sync, `ALL_TOOLS` has 88 built-in tools (90 registered with factory `mount_mcp`/`tool_search`) and `SLASH_COMMANDS.length` reports 99 commands. Latest full verify: 471 TS test files / 3695 tests. Older counts in session history are milestones, not current truth.
...
**VANTA-HOOK-EVENTS (building).** `.vanta/hooks.json` now accepts the full 30-event hook vocabulary, and live code emits new session, display, tool-batch, permission, compaction, config, worktree, fleet, and subagent events. Remaining owners are explicit WIP: file/cwd watcher events, MCP elicitation/notification events, `StopFailure`, and `TeammateIdle`. Verified: **471 TS files / 3695 tests green**, `tsc` clean, size gate clean.
...
- **Hook engine** (`hooks/`) — `.vanta/hooks.json` accepts the 30-event hook vocabulary and fires hooks on lifecycle, prompt/display, tool, permission, compaction, config, worktree, fleet, and subagent paths. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; JSON context on stdin/body/prompt, regex matchers for tool/prompt events, plus `maintenance` and `sessionType` filters. Agent hooks use injected `AgentDeps` on live tool/prompt/stop/session paths; early lifecycle hooks are providerless. File/cwd watcher, MCP elicitation/notification, `StopFailure`, and `TeammateIdle` owners remain WIP. Distinct from the in-process `plugins/hooks.ts` JS bus.
