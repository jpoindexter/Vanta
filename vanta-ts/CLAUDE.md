...
| `hooks/` | VANTA-HOOK-TYPES + VANTA-HOOK-AGENT-TYPE — `.vanta/hooks.json` schema + execution. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; shared controls: `timeoutMs`, `once`, `statusMessage`. Agent hooks are dependency-injected for live tool/prompt/stop events to avoid agent-loop import cycles. |
...
- **Current source counts beat historical session counts.** As of the 2026-06-18 context sync, `ALL_TOOLS` has 88 built-in tools (90 registered with factory `mount_mcp`/`tool_search`) and `SLASH_COMMANDS.length` reports 99 commands. Latest full verify: 471 TS test files / 3690 tests. Older counts in session history are milestones, not current truth.
...
- **Hook engine** (`hooks/`) — `.vanta/hooks.json` fires hooks on Setup / SessionStart / PreToolUse (non-zero BLOCKS, fail-closed, after the kernel gate) / PostToolUse / UserPromptSubmit / Stop. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; JSON context on stdin/body/prompt, regex matchers for tool/prompt events, plus `maintenance` and `sessionType` filters. Agent hooks use injected `AgentDeps` on live tool/prompt/stop paths; early lifecycle hooks are providerless. Wired in `agent/dispatch-tool.ts` + `interactive.ts`. Distinct from the in-process `plugins/hooks.ts` JS bus.
