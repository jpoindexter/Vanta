...
- **Current source counts beat historical session counts.** As of the 2026-06-19 context sync, `ALL_TOOLS` has 93 built-in tools (96 registered with factory `mount_mcp`/`tool_search`/`mcp_auth`) and `SLASH_COMMANDS.length` reports 99 commands. Latest full verify: 558 TS test files / 4466 tests. Older counts in session history are milestones, not current truth.
