...
- **Design-token + glyph layer (P1):** `tui/figures.ts` (named glyphs ⏺ ❯ ✻ ✔ ✘ ▶ + ASTERISK_FRAMES + verbs). `tui/theme.ts` gained semantic tokens (success/error/warning/info/marker/userMarker; `primary` = text/white, identity in accent/border/marker). Glyphs wired through `transcript.tsx` (⏺ assistant marker, ❯ user, ✻ boundary), `status-bar.tsx` (● ready), `spinners.ts` (`asterisk` cycle).
- **Interactive footer (P2):** `tui/footer-hint.tsx` — one dim line above the composer: ○/◐/● permission mode · model · "? for shortcuts".
- **Keybinding registry (P3):** `tui/keybinding/` — `chord.ts` (pure parse/match/format: `ctrl+o`→`^O`, `shift+tab`→`⇧⇥`), `registry.ts` (`DEFAULT_BINDINGS` grouped by `KeyContext`: global/transcript/composer/palette/at-palette/modal/tabs), `use-keybinding.ts` (`useKeybinding(action, handler, {isActive})` + cached merged chord map + `resetKeybindingCache`), `user-bindings.ts` (optional `~/.vanta/keybindings.json` overrides, zod-validated, fail-soft). `use-keybindings.ts` + `use-scroll-keys.ts` migrated onto it; `help-overlay.tsx` renders from the registry. (Composer emacs/readline + history edge-fall-through already existed.) **Not** done vs the stricter CC-KEYBINDING-* roadmap cards: no hot-reload watcher, no `/doctor` report, no conflict warnings, 7 contexts (not 17+).
- **Layout & mission-control (P4, opt-in):** `tui/layout.tsx` `FullscreenLayout` (scrollable + bottom slots) + `tui/bottom-chrome.tsx` (the modal/composer slot router, extracted from app.tsx → app.tsx 303→~190 lines, App fn back under the gate). `tui/mission-control/` — tabbed Kernel verdict ladder / live Goals / live Loops (`gatherCockpitData`: getGoals + listDefs/loadState/openEscalations, best-effort), opened via `/cockpit`, switched by the `tabs` keybinding context. `tui/new-messages*` — "N new messages" pill when scrolled up (slow-poll the ScrollBox position). `transcript.tsx` `toolGroupRole` brackets consecutive tool calls into one ┌│└ block. ⇧⇥ approval-cycle suppressed while an overlay is open so tabs owns ⇧⇥. **Note:** this is a lighter opt-in overlay, NOT the TUI-V2 card's always-on 3-column rail surface (that card stays open).
