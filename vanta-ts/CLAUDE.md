...
| `hooks/` | VANTA-HOOK-TYPES + VANTA-HOOK-AGENT-TYPE + VANTA-HOOK-EVENTS (shipped) — `.vanta/hooks.json` 30-event schema + execution. Types: `command`/`shell`, `http`, `mcp_tool`, `prompt`, `agent`; shared controls: `timeoutMs`, `once`, `statusMessage`. Agent hooks are dependency-injected for live tool/prompt/stop/session events to avoid agent-loop import cycles. |
