...
**CC-INIT-CMD.** `/init [--force|--print]` now generates `.claude/CLAUDE.md` for the current project. It detects common manifests, visible top-level structure, and existing `AGENTS.md`/`CLAUDE.md`/`README.md`/`ROADMAP.md` hints; it refuses to overwrite by default. `roadmap.json` marks `CC-INIT-CMD` shipped. **93 slash commands.**
