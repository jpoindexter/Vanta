...
2. **Nearest-level AGENTS.md.** Update the file map, decisions, and gotchas for the layer you touched — `vanta-ts/AGENTS.md` for agent work, root `AGENTS.md` for kernel work. One line per file/fact.

3. **New directory level → new AGENTS.md.** If you created a new layer or package, add a `AGENTS.md` there: one-line role, file map, key decisions, how-to-extend, commands. Link up to the parent.

4. **Root AGENTS.md status + maps.** Update the `## Status` line (current phase, test count, what's next) and the kernel module/API tables if the kernel changed.
...
7. **Locked decisions.** Add to the layer AGENTS.md "Key decisions (don't re-litigate)" section: the choice + a one-line why. This is what stops future re-debate.
...
- If a AGENTS.md exceeds ~120 lines, it's carrying explanation that belongs in code comments or the PRD — trim it.
- Global `~/.Codex/AGENTS.md` conventions are inherited, never repeated.
