...
Document the architectural decisions that should constrain what gets built. These prevent Codex from re-deriving foundational decisions from scratch each session.
...
Create a minimal session template for every Codex session during the MVP stage. Starting each session without this is how architectural drift accumulates.

```markdown
## Session: [Date] — [Goal]

**Context:** [Link to scope document] + [Link to AGENTS.md]
...
- Whether anything needs to be added to AGENTS.md
...
If all three are yes → add it to scope with evidence cited, update AGENTS.md, continue.
...
- The scope document is created BEFORE Codex writes the first line of production code.
