...
- **ND-first, useful to anyone** — `/support` lets the operator explicitly set current capacity, memory load, activation, and motivation. Vanta can preserve a functional minimum, externalize task state, or create one truthful initiation bridge without inferring a diagnosis or importing a private biography. Seven reusable patterns ship in [`executive-function-skills/`](executive-function-skills/).
