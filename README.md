...
- **ND-first, useful to anyone** — executive-function support is built into Vanta's core system prompt and applies automatically: orient, reduce choices, externalize state, start one reversible action, and preserve re-entry. `/support` is an optional explicit override for current capacity, memory load, activation, and motivation. The reusable [`executive-function-skills/`](executive-function-skills/) pack exports the patterns to other agents; Vanta does not depend on installing or invoking it.
