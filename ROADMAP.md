...
and **93 slash commands**. Last recorded full verify: **3334 TS tests**, `tsc` clean,
kernel tests green.
...
resume forking, the opt-in TUI v2 mission-control shell, the Vite/React desktop renderer, per-tool permission request UIs, memory guardrails, per-task tool scoping, solutioning mode, and the opt-in runtime plugin framework are live slices; browser OS-level
