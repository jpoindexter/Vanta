...
- [~] **P1 · Slash-command parity** — conversation-level set SHIPPED 2026-06-02 (commit pending): `/history` (transcript), `/retry` (drop last turn + re-send), `/undo` (drop last turn), `/reset` (alias of /clear). Wired in BOTH readline + TUI via a shared send path + `resend` signal; 6 tests; 456 green, tsc clean. **P1b remaining (needs session-store work): `/title <name>` (title override in `saveSession`), `/fork` (branch → new session id + copy history), `/redraw` (TUI repaint).** Known gap: TUI `/undo` retracts model history but not the rendered transcript (needs a reducer action) — readline is fully correct.
