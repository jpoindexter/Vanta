...
- [ ] **INSTALL · One-line curl install** (S) — close the last Hermes/OpenClaw install gap. Argo already has the local half (`./install.sh` builds + installs the global `argo` command). Missing: a **hosted `bootstrap.sh`** so `curl -sSL <url> | bash` works on a clean machine with nothing cloned. Bootstrap = `git clone` (or tarball fetch) into a fixed source dir → `cd` → run the existing `install.sh`. Needs the **`/dev/tty` open-based probe** (Hermes regression #16746: `if (: </dev/tty) 2>/dev/null; then … < /dev/tty`) so a piped (non-interactive) install doesn't crash on the setup prompt — it should either skip setup with guidance or read from `/dev/tty` when one is attachable. **Done =** `curl -sSL <url> | bash` on a fresh machine installs argo and `argo setup` runs after. *(Requires: repo public for `raw.githubusercontent.com`, or a token path; fixed clone dir.)*
