...
- **`./install.sh`** — Hermes/OpenClaw-style installer: builds kernel + deps, seeds `~/.vanta/skills`, installs a global **`vanta`** launcher into `~/.local/bin` (only edits a shell rc if that dir isn't already on PATH). Type `vanta` from anywhere.
...
- [x] **A4 · `vanta setup` wizard** (M) — provider picker → hidden key prompt → **merge** into `argo-ts/.env` (pure `upsertEnv`, preserves all other keys) → model w/ default → persist (0600). Unit + integration tested.
