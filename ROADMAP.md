...
- [x] **MCP-1 · Use any MCP (consume)** (S) — ✅ SHIPPED 2026-06-04. `readMcpConfig` now accepts Claude's `mcpServers` key (alongside `servers`) and discovers `./.mcp.json` in the project (project wins over `~/.argo/mcp.json`; `ARGO_MCP_SERVERS` still wins inline). `argo mcp list` lists configured servers. Config discovery done + unit-tested (+4 tests); live consume needs a real server.
- [x] **MCP-2 · Make + hook in at runtime** (M) — ✅ SHIPPED 2026-06-04. **`mount_mcp` tool** (`tools/mount-mcp.ts`): `buildMountMcpTool(registry)` factory spawns an MCP server, mounts its tools into the LIVE registry mid-session, kernel-gated via `assess()` (`describeForSafety` → "spawn mcp server …"). **`build-mcp-server` skill** (`skills-library/`): teaches Argo to scaffold a TS MCP server with the SDK, build it, and mount it. **Done =** mount_mcp registers a server's tools into the running registry; skill covers scaffold-on-demand.
- [x] **MCP-3 · Be a server (serve)** (M/L) — ✅ SHIPPED 2026-06-04. **`argo mcp serve`** (`mcp/server.ts`): exposes Argo's tools over MCP stdio (JSON-RPC 2.0, mirror of `client.ts`). Every `tools/call` routes through the kernel `assess()` gate — `block`/`ask` refused as `isError` results (headless: no human to prompt), only `allow` executes. Read-only **allowlist** (`ARGO_MCP_SERVE_TOOLS`, defaults to 9 read-only tools) bounds exposure; the kernel is the real boundary. `console.log→stderr` keeps the protocol stream clean. **Done (live-verified):** a real `McpClient` drove the `argo mcp serve` subprocess end-to-end — handshake, `tools/list` (filtered), `read_file` allow→executed, out-of-scope refused, non-allowlisted refused. *Claude Code integration itself untested (offline-verified via reciprocal client, house discipline like E2/E5).* Supersedes **E6** (ACP wrapper).
...
- [ ] **SCOPE-1 · Writable zones beyond the repo** (M) — `write_file` (`tools/write-file.ts:48`) hard-refuses any path outside `ARGO_ROOT` via `resolveInScope`, so "put it on my Desktop" degrades into write-in-repo → `shell_cmd` copy → repo pollution. But the kernel *already* classifies an out-of-root absolute path as **Ask**, not Block — the tool is stricter than the boundary it mirrors. **Fix:** configurable writable zones (`ARGO_WRITABLE_DIRS`, e.g. `~/Desktop` + a workspace dir). A path inside a zone but outside the repo → route through `ctx.requestApproval` (→ kernel Ask) instead of refusing; `scope.ts` and the Rust `inside_scope` stay in agreement; out-of-zone stays Ask/Block. **Open decision:** bounded zones (safer, recommended) vs per-write approval for any path (more flexible, yes-fatigue risk). **Done =** Argo writes directly to `~/Desktop` with one approval — no repo pollution, no shell copy.
- [ ] **O10 · Autonomy ladder (L1–L5, kernel-bounded)** (L) — formalize the factory's autonomy as one selectable level, replacing today's binary review-vs-auto. Requirements: (1) `ARGO_AUTONOMY_LEVEL` setting read by `factory/run.ts`; (2) per-level stop gates — **L1** suggest only · **L2** branch+edit+test, show diff, stop · **L3** commit, no push · **L4** push, no merge · **L5** auto-merge low-risk; (3) **L6 (edit safety-critical code) is NOT grantable via config** — kernel `is_protected_path` blocks kernel/factory/manifesto writes (out-of-band approval only); (4) a **low-risk classifier** for L5 (non-protected TS only, all tests pass, no dep/schema/env/migration change, diff under a bound); (5) observability — every autonomous action → `events.jsonl` + review-summary artifact; (6) reversibility — branch-per-change, never force-push, never touch the default branch without approval. **Done =** set level N → the loop stops exactly at gate N; protected paths still prompt at every level. *(Foundation exists: O9 factory `review`/`auto` modes + kernel protected paths.)*
