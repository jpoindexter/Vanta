...
- **Last recorded full verify:** 3695 TS tests green (471 files), `tsc` clean, kernel tests green (see `vanta-ts/CLAUDE.md` 2026-06-18 notes)
...
- **Hooks:** `vanta-ts/src/hooks/` owns `.vanta/hooks.json`; supported hook types are `command`/`shell`, `http`, `mcp_tool`, `prompt`, and `agent`, with shared `timeoutMs`, `once`, and `statusMessage`. The schema accepts the 30-event hook vocabulary; most live agent/session/tool/worktree/fleet/permission/compaction/config events are wired, while watcher-only file/cwd, MCP elicitation/notification, stop-failure, and teammate-idle owners remain WIP under `VANTA-HOOK-EVENTS`.
